"""
Experiment: Tensor Sim vs Functional Sim Across Distributions (No Residual)

Same as run_experiment.py but using pure bilinear models (no skip connection).
This isolates the bilinear tensor so tensor similarity should theoretically
match functional similarity for Gaussian inputs.

Also uses continuous cosine similarity as the functional metric instead of
discrete argmax agreement.
"""
...
class PureBilinear(nn.Module):
    """Multi-layer bilinear network WITHOUT residual connections.
    Keeps same .Ls/.Rs/.Ds/.num_layers interface so tensor_similarity works."""
    def __init__(self, n, num_layers=3, rank=32):
        super().__init__()
        self.n = n
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            Lh = h @ self.Ls[i].T
            Rh = h @ self.Rs[i].T
            bilinear_out = (Lh * Rh) @ self.Ds[i].T
            h = bilinear_out  # NO residual: h = bilinear(h), not h + bilinear(h)
        return h


# --- Config ---
NUM_SEEDS = 5
NUM_LAYERS = 1  # single layer for clean theory comparison
RANK = 32
...
RESULTS_DIR = 'results_no_residual'
...
def continuous_func_sim(m1, m2, dist_fn, n=4, num_samples=50000):
    """Cosine similarity of flattened output vectors."""
    m1.eval(); m2.eval()
    with torch.no_grad():
...
plt.suptitle('Training Curves by Distribution (No Residual, 1 Layer)', fontsize=16)
plt.tight_layout()
...
for idx, dist_name in enumerate(DISTRIBUTIONS):
    ...
    ax.set_ylabel('Func Sim (continuous, train dist)')
    ax.grid(True, alpha=0.3)
    if idx == 0:
...
ax.set_title('Standard vs Generalized Tensor Similarity — No Residual')
ax.legend()
...
ax.set_xlabel('Tensor Sim (standard)')
ax.set_ylabel('Func Sim (Gaussian, continuous)')
ax.set_title(f'Cross-Distribution — No Residual (r={r_cross:.3f})')
ax.grid(True, alpha=0.3)
plt.tight_layout()
