"""TN similarity with numpy backend to avoid torch memory fragmentation.

This is a modified copy of src/components/similarity.py that uses numpy
for tensor contractions to avoid the memory fragmentation issue that causes
141GB+ memory usage for n_ctx=8 models.

The fix: convert torch tensors to numpy before cotengra contractions,
then convert back to torch at the end.
"""
import torch
import numpy as np
...
def _to_numpy(t):
    """Convert tensor to numpy, handling torch tensors."""
...
def _to_torch(arr, like):
    """Convert numpy array back to torch tensor with same device/dtype as 'like'."""
    return torch.from_numpy(np.ascontiguousarray(arr)).to(device=like.device, dtype=like.dtype)
...
def _contract(tn, bridges, output_inds):
    ...
    Uses numpy backend internally to avoid torch memory fragmentation.
    """
    ...
    # Convert all tensor data to numpy for contraction (avoids torch memory fragmentation)
    tn_data = [_to_numpy(t.data) for t in tn]
    bridge_data = [_to_numpy(b.data) for b in bridges]
    
    return _EXPR_CACHE[key](*tn_data, *bridge_data)


def _isserlis(tn, legs, S_for_pair, mu_for_leg, output_inds, like):
    ...
    Args:
        like: A torch tensor to match device/dtype for the output.
    """
    def bridge(l1, l2):
        return Tensor(S_for_pair(l1, l2), inds=l1[:2] + l2[:2])

    # Accumulate in numpy to avoid torch memory fragmentation
    result = sum(
    ...
    result = result - (n_matchings - 1) * _contract(tn, mu_bridges, output_inds)
    
    # Convert back to torch
    return _to_torch(result, like)
...
def similarity(model_a, model_b):
    ...
    This version uses numpy backend for contractions to avoid torch memory fragmentation.
    Memory usage is ~2GB instead of ~141GB for n_ctx=8 models.
    """
