#!/usr/bin/env python3
"""Plot layer-1 head TN heatmaps, individually and as a compact grid."""
...
def plot_grid(values, labels, output_path, *, vmin, vmax, cmap):
    n_heads = len(labels)
    fig, axes = plt.subplots(n_heads, n_heads, figsize=(2.65 * n_heads, 2.65 * n_heads), constrained_layout=True)
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Path to l1_head_tn_sims.npz")
    parser.add_argument("--output_dir", default=None)
    parser.add_argument("--raw", action="store_true", help="Plot raw TN values instead of local cosine sims.")
    parser.add_argument("--vmin", type=float, default=-1.0)
    parser.add_argument("--vmax", type=float, default=1.0)
    parser.add_argument("--cmap", default="coolwarm")
    parser.add_argument("--grid", action="store_true", help="Also write one compact HxH image containing all heatmaps.")
    args = parser.parse_args()

    input_path = Path(args.input)
    data = np.load(input_path, allow_pickle=True)
    steps = data["steps"]
    labels = [str(x) for x in data["head_labels"]]
    key = "l1_head_tn_values" if args.raw else "l1_head_tn_sims"
    values = data[key]
    mode = "raw" if args.raw else "local"
    output_dir = Path(args.output_dir) if args.output_dir else input_path.with_suffix("").parent / "l1_head_tn_heatmaps"
    ...
    for gi, g in enumerate(labels):
        for hi, h in enumerate(labels):
            title = f"{g} vs {h} L1-head TN {'raw' if args.raw else 'local cosine'}"
    ...
    if args.grid:
        grid_path = output_dir / f"{mode}_all_l1_heads.png"
