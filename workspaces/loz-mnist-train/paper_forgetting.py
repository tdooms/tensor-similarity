...
from functions.tn_sim import tn_sim_thomas
...
for seed in seeds:
    ...
    for stage in digit_curriculum:
        ...
        for cp in tqdm(checkpoints, desc=f"{seed} {name}"):
            ...
            results[seed]['functional'].append(tn_sim_thomas(model_temp, reference_model))
...
for i in tqdm(range(N_HEATMAP)):
    for j in range(i + 1):
        s = tn_sim_thomas(heatmap_models[i], heatmap_models[j])
        heatmap[i, j] = s
        heatmap[j, i] = s
...
plt.tight_layout()
plt.savefig(figures_dir / "paper_forgetting_accuracy.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(figures_dir / "paper_forgetting_loss.png", dpi=150, bbox_inches='tight')
plt.show()
...
ax_sim.set_ylabel("Tensor similarity")
ax_sim.set_ylim(-0.05, 1.05)
ax_sim.set_xlim(0, cb[-1])
ax_sim.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(figures_dir / "paper_forgetting_similarity.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(figures_dir / "paper_forgetting_heatmap.png", dpi=150, bbox_inches='tight')
plt.show()
print("Done.")
