...
from .attention import QuadraticAttention, BilinearAttention, CubicAttention, SoftmaxAttention

ATTN_REGISTRY = {
    "quadratic": QuadraticAttention,
    "bilinear": BilinearAttention,
    "cubic": CubicAttention,
...
class AttentionLM(nn.Module):
    ...
    Supports attn_type: 'quadratic', 'bilinear', 'cubic', 'softmax'.

    norm_type: 'none', 'rmsnorm', 'layernorm'
    norm_place: 'none', 'post_embed', 'pre_layer', 'pre_unembed'
    """

    def __init__(
        self,
        vocab_size, n_ctx, d_model, n_head, n_layers,
        attn_scale=0.2, rope_base=10000, use_rmsnorm_qk=False,
        use_bias_qkv=True, use_bias_o=True,
        std_embed=0.02, std_qkv=0.02, std_o=0.01,
        attn_type="quadratic", norm_type="rmsnorm", norm_place="pre_unembed",
    ):
        super().__init__()
        assert norm_type in NORM_TYPES
        assert norm_place in NORM_PLACES
    ...
    def _init_weights(self, std_embed, std_qkv, std_o):
        ...
        for layer in self.layers:
            ...
            if hasattr(layer, 'q2') and layer.q2.bias is not None:
