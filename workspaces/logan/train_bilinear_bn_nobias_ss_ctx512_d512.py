#!/usr/bin/env python3
"""Bilinear attention + BatchNorm, NO BIASES, trained on SimpleStories (d=512, ctx=512).

Architecture: 2L attention-only, d_model=512, 16 heads, bilinear with BatchNorm on Q1,K1,Q2,K2
All linear layers have bias=False. Same as Run D but on SimpleStories data.
Data: SimpleStories (story-aligned, padded/truncated), vocab=4019, loops for 5B total

Usage:
    python train_bilinear_bn_nobias_ss_ctx512_d512.py
    python train_bilinear_bn_nobias_ss_ctx512_d512.py --batch-size 96
"""
import math
import json
import time
...
SS_TRAIN_PATH = CACHE_DIR / "ss_aligned_train_ctx512.pt"
SS_TEST_PATH = CACHE_DIR / "ss_aligned_test_ctx512.pt"
...
class BilinearBatchNormAttention(nn.Module):
    ...
    def __init__(self, d_model, n_head, n_ctx, scale=1.0, rope_base=10000):
        ...
        # Two QK circuits — all bias=False
...
class CachedDataset(Dataset):
    """Map-style dataset from a cached .pt file of token chunks."""
...
def main(batch_size=64, use_compile=True):
    ...
    print(f"=== Bilinear + BatchNorm + NO BIAS + SimpleStories: ctx={N_CTX}, d={d_model} ===")
    print(f"2L attn-only, d={d_model}, {n_head} heads (d_head={d_model//n_head}), bilinear+BN, no bias")
    print(f"Data: SimpleStories (cached), vocab={VOCAB_SIZE}")
    ...
    assert SS_TRAIN_PATH.exists(), f"Cached SS train not found: {SS_TRAIN_PATH}"
    assert SS_TEST_PATH.exists(), f"Cached SS test not found: {SS_TEST_PATH}"

    train_ds = CachedDataset(SS_TRAIN_PATH, n_ctx=N_CTX)
    val_ds = CachedDataset(SS_TEST_PATH, n_ctx=N_CTX, max_samples=500)
    train_dl = DataLoader(train_ds, batch_size=batch_size, shuffle=True, drop_last=True)
    val_dl = DataLoader(val_ds, batch_size=batch_size, shuffle=False, drop_last=True)

    n_train_chunks = len(train_ds)
    tokens_per_epoch = n_train_chunks * N_CTX
    n_epochs_needed = math.ceil(TARGET_TOKENS / tokens_per_epoch)
    print(f"Train chunks: {n_train_chunks:,} ({tokens_per_epoch/1e9:.3f}B tokens/epoch)")
    print(f"Epochs needed: ~{n_epochs_needed} to reach {TARGET_TOKENS/1e9:.0f}B tokens")
    ...
    # Pre-compute induction eval data (using SS test set)
    val_data = torch.load(SS_TEST_PATH, weights_only=True).to(torch.long)[:, :N_CTX]
    ...
    run_dir = Path(f"runs/{timestamp}_bilinear_bn_nobias_ss_ctx512_d512")
    ...
    while step < max_steps:
        ...
        for batch in train_dl:
            ...
            if step % 10 == 0:
                ...
                log({"train_loss": loss.item(), "lr": scheduler.get_last_lr()[0], "epoch": epoch}, step)
                if step % 100 == 0:
                    print(f"  step {step}/{max_steps}  loss={loss.item():.4f}  "
                          f"tokens={tokens_so_far/1e9:.3f}B  epoch={epoch}  elapsed={elapsed:.0f}s")
            ...
            if step % 2500 == 0:
                ...
                log({"val_loss": val_loss, "epoch": epoch}, step)
                model.train()
    ...
    toy = eval_toy_repeated(model, VOCAB_SIZE, device, n_samples=100, half_len=16)
    indist = eval_indist_bigrams(model, val_data[:100], N_CTX, bigram_positions, device)
    log({"val_loss": val_loss, **toy, **indist, "tokens_seen_B": step * tokens_per_step / 1e9, "epoch": epoch}, step)
